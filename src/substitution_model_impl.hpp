...
void substitution_model<T, log_space>::init(const std::string& file_stem)
{
	...
	if (verbose)
	{
		std::cerr << rate_matrix_file_name << '\n';
	}
	...
	if (verbose)
	{
		std::cerr << "Sum of probability vector: "
		...
		const std::size_t matrix_rows)
	{
		...
		for (std::size_t i = 0; i < matrix_rows; ++i)
		{
			for (std::size_t j = 0; j < N; ++j)
			{
				std::cerr << std::right << std::setw(8) << matrix[i][j];
			}
			std::cerr << '\n';
		}
	};
	...
	if (verbose)
	{
		std::cerr << "Average substitution rate before normalisation: " << sub_rate << '\n';
	}
	for (double* i = &m_data_matrix[0][0]; i < &m_data_matrix[0][0] + N * N; ++i)
	{
		(*i) /= sub_rate;
	}
	...
	if (verbose)
	{
		std::cerr << "Average substitution rate after normalisation: " << sub_rate << '\n';
	...
	: m_rate(rate), m_time(-1), m_cached(false)
{
...
#ifndef NDEBUG
			std::cerr << "Looking for parameter files in " << i << "/... "
					  << (found_file ? "yes" : "no") << '\n';
#endif
			if (found_file)
			{
	...
	if (verbose)
	{
		std::cerr << "Average substitution rate = " << m_rate << '\n';
	}
	...
	const population<T>& recipient_population, double time) const
{
	...
	if (verbose)
	{
		std::cerr << std::right << std::scientific << std::setprecision(2) << std::setw(col_width) << "";
		for (const auto& i : transmitter_population)
		{
			std::cerr << std::setw(col_width) << i.m_name;
		}
		std::cerr << '\n';
	}
	...
	for (auto it = recipient_population.cbegin(); it != it_end_recip; ++it)
	{
		...
		if (verbose)
		{
			std::cerr << std::setw(col_width) << it->m_name;
		}
...
#ifndef NDEBUG
	std::cerr << "Vector of Transmitter Counts:\n";
	for (const auto& i : transmitter_population)
	{
		std::cerr << '\t' << i.m_count;
	}
	...
	bool display_raw) const
{
	...
	for (std::size_t j = 0; j < N; ++j)
	{
		...
		for (std::size_t i = 0; i < N; ++i)
		{
			...
			std::cerr << T::index_to_elem(i) << ':' << entry_ij << ' ';
			sum += entry_ij;
		}
	...
	std::cerr << "Average substitutions per time per site: " << average_sub_rate
			  << '\n';
}
...
#pragma omp critical
	{
#ifndef NDEBUG
		std::cerr << "Resetting cache (time[" << time << "] != m_time[" << m_time
				  << "])\n";
#endif
